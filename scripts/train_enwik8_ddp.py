...
if is_main_process and start_step == 0:
    ...
    log_path.write_text(
        "step,lr,train_loss,val_loss,val_bpc,"
        "step_seconds,optimizer_steps_per_sec,"
        "micro_batches_per_sec,tokens_per_sec,tokens_per_sec_per_gpu\n",
        encoding="utf-8",
    )
...
def write_log(
    ...
    with log_path.open("a", encoding="utf-8") as f:
        f.write(
            f"{step},{lr},{train_loss},{val_loss},{val_bpc},"
            f"{step_seconds},{optimizer_steps_per_sec},"
            f"{micro_batches_per_sec},{tokens_per_sec},"
...
for step in range(start_step, num_steps):
    ...
    global_tokens_per_step = batch_size * world_size * grad_accum_steps * seq_len
    tokens_per_sec = global_tokens_per_step / step_seconds
