...
def parse_args():
    ...
    parser.add_argument("--seq-len", type=int, default=3071)
    ...
    parser.add_argument("--output-path", type=str, default="outputs/aft_cifar10_ar.pt")
    parser.add_argument("--log-path", type=str, default="outputs/train_cifar10_ar_log.csv")
...
data_dir = Path("data/cifar10/processed")
...
if seq_len > 3071:
    raise ValueError("seq_len must be <= 3071 for CIFAR10 autoregressive modeling")
...
def make_batch(data, batch_size, seq_len, device):
    # 每个 rank 都会独立随机抽图像；DDP 会在反向传播时自动同步各 rank 的梯度。
    # 每张 CIFAR10 图片已经被预处理成 [3072] 的 byte 序列。
    # x: [B, T] 是前 T 个 byte；y: [B, T] 是向后错一位的预测目标。
    indices = torch.randint(0, len(data), (batch_size,))
    sequences = data[indices] #[B, 3072]
...
if is_main_process and start_step == 0:
    ...
    log_path.write_text("step,train_loss,val_loss,val_bpd\n", encoding="utf-8")
...
def estimate_loss(data, num_batches=20):
    ...
    for _ in range(num_batches):
        ...
        with torch.amp.autocast("cuda", enabled=amp):
            logits = model(x)
            # logits: [B, T, 256]，因为每个 byte 的类别范围是 0..255。
            # CrossEntropyLoss 需要 [B*T, 256] 和 [B*T]。
    ...
    torch.save(
        {
            ...
            "config": {
                "dataset": "cifar10",
                "task": "autoregressive_image_modeling",
                "vocab_size": vocab_size,
                "seq_len": seq_len,
...
def write_log(step, train_loss, val_loss, val_bpd):
    # 日志同样只让主进程写，避免多进程重复写入。
    if not is_main_process:
        return
    with log_path.open("a", encoding="utf-8") as f:
        f.write(f"{step},{train_loss},{val_loss},{val_bpd}\n")
...
for step in range(start_step, num_steps):
    ...
    if step % eval_interval == 0:
        ...
        # BPD = bits per dimension，图像自回归里常用的比特指标。
        # 这里每个 dimension 是一个 byte 位置，nats 除以 ln(2) 得到 bits。
        val_bpd = val_loss / math.log(2)
        ...
        if is_main_process:
            ...
            write_log(step, train_loss, val_loss, val_bpd)
